...
public class Personnage extends Humain {
	...
	public void ajouterInventaire(Objet o) {
		...
		if (poidsInventaire() != inventaire.length) {
			for (int i = 0; i < poidsInventaire(); i++) {
				// s'il s'agit d'un objet qui existe deja dans l'inventaire, on rajoute juste
				// les quantité
			...
			// si l'objet n'a pas encore été ajouté à l'inventaire apres le test précédant,
			// on ajoute l'objet
			if (deja_ajoute != 1) {
				int b = placeLibre();
				inventaire[b] = o;
	...
	// utile pour les objet a usage unique ou objet que l'on doit utiliser qu'une
	// fois pour certaines actions
	public void retirerObjet(int place) {
		...
		if (inventaire[place].getQuantite() == 0) {
			System.out.println(1 + " " + inventaire[place].getNom()
					+ " a été retiré de l'inventaire. Il ne vous en reste plus...");
			inventaire[place] = null;
			reorganise(place);
		} else {
			System.out.println(1 + " " + inventaire[place].getNom() + " a été retiré de l'inventaire. Il vous en reste "
					+ inventaire[place].getQuantite());
		}
	}
	...
	public void retirerObjetMultiple(int nombre, int place) {
		...
		if (inventaire[place].getQuantite() == 0) {
			...
			System.out.println(nombre + " " + inventaire[place].getNom()
					+ " a été retiré de l'inventaire. Il vous en reste " + inventaire[place].getQuantite());
		}
	}
	 ...
	 * 
	 * @param a
	 *            a est un indice a partir duquel on commence à réorganiser si
	 *            l'inventaire est remplit et qu'il y a un emplacement vide entre
	 *            les emplacement utilisé on decale les objets de la meme maniere
	 *            qu'une liste
	 */
	...
	/**
	 * Cherche une place libre dans l'inventaire Renvoie l'indice d'une place libre
	 * dans l'inventaire
	 */
	...
	// a specifier
	public void utiliserObjet(Objet objet, int emplacement) {
		String type = objet.getClass().getSimpleName();

		switch (type) {
		case "ObjetVital":
			seSoigner((ObjetVital) objet);
			System.out.println(objet.getNom() + " utilisé(e)");
			retirerObjet(emplacement);

			break;

		case "Arme":
			setArme((Arme) objet);
			System.out.println(objet.getNom() + " équipé(e)");
			break;
		case "ObjetDivers":
			System.out.println("Objet inutilisable maintenant");
			break;
		default:
	...
	/**
	 * Reçoit les dégats de l'adversaire Prend en compte la posture et la défense du
	 * personnage puis l'attaque du monstre.
	 */
	...
	/**
	 * Permet au personnage de se mettre en posture défensive Ce qui aura pour effet
	 * de reduire considérablement les degats !
	 */
